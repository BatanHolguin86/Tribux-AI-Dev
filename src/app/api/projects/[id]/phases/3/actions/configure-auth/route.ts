...
export async function POST(
  ...
  try {
    ...
    if (projectError) {
      console.error('[configure-auth] Project query error:', projectError)
    }
    ...
    const result = streamText({
      ...
      onFinish: async ({ text, usage }) => {
        await recordStreamUsage({ userId: user.id, projectId, eventType: 'action_configure_auth', model: DEFAULT_MODEL_ID, usage })
        try {
            ...
            if (filesToCommit.length > 0) {
              try {
                const commitResult = await commitMultipleFiles(
                  project.repo_url,
                  filesToCommit,
                  'feat: configure authentication — RLS policies & auth helpers (Tribux AI Phase 03)',
                )
