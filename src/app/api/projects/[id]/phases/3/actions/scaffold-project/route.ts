...
export async function POST(
  ...
  try {
    ...
    if (projectError) {
      console.error('[scaffold-project] Project query error:', projectError)
    }
    ...
    const result = streamText({
      ...
      onFinish: async ({ text, usage }) => {
        await recordStreamUsage({ userId: user.id, projectId, eventType: 'action_scaffold_project', model: DEFAULT_MODEL_ID, usage })
        try {
