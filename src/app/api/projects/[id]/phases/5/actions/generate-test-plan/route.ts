...
import { generateTestPlanPrompt } from '@/lib/ai/prompts/action-prompts'
...
export async function POST(
  ...
  const actionDef = getActionByName('generate-test-plan')!
  let executionId = ''
  ...
  try {
    ...
    // 6. Build full project context (includes feature specs + repo tree)
    const context = await buildFullProjectContext(projectId)

    // 7. Build prompt
    const systemPrompt = generateTestPlanPrompt({
    ...
    const result = streamText({
      ...
      messages: [
        {
          role: 'user' as const,
          content: 'Generate a comprehensive test plan based on the feature specs and acceptance criteria.',
        },
      ],
      ...AI_CONFIG.documentGeneration,
      onFinish: async ({ text, usage }) => {
        await recordStreamUsage({ userId: user.id, projectId, eventType: 'action_generate_test_plan', model: DEFAULT_MODEL_ID, usage })
        try {
          // 9. Extract markdown file from AI output
          ...
          const commitResult = await commitMultipleFiles(
            project.repo_url!,
            filesToCommit,
            'docs: add test plan (Tribux AI Phase 05)',
          )
          ...
          console.error('[generate-test-plan] Post-stream error:', error)
