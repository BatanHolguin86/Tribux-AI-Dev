  ...
  try {
    ...
    const result = streamText({
      ...
      onFinish: async ({ text, usage }) => {
        await recordStreamUsage({ userId: user.id, projectId, eventType: 'action_generate_qa_report', model: DEFAULT_MODEL_ID, usage })
        try {
          ...
          if (extractedFiles.length > 0 && project.repo_url) {
            ...
            await commitMultipleFiles(
              project.repo_url,
              filesToCommit,
              'docs: add QA report (Tribux AI Phase 05)',
            )
          }
