...
export async function POST(
  ...
  const actionDef = getActionByName('generate-ops-runbook')!
  let executionId = ''
  ...
  try {
    ...
    // 7. Build prompt
    const systemPrompt = generateOpsRunbookPrompt({
    ...
    const result = streamText({
      ...
      messages: [
        {
          role: 'user' as const,
          content: 'Generate the deployment runbook, production architecture doc, and troubleshooting guide.',
        },
      ],
      ...
      onFinish: async ({ text, usage }) => {
        ...
        try {
          ...
          const commitResult = await commitMultipleFiles(
            project.repo_url!,
            filesToCommit,
            'docs: add ops runbook, architecture, and troubleshooting (Tribux AI Phase 06)',
          )
          ...
          await recordActionComplete(
            executionId,
            'success',
            `Committed ${commitResult.filesChanged} documentation files`,
            {
          ...
          console.error('[generate-ops-runbook] Post-stream error:', error)
    ...
    console.error('[generate-ops-runbook] Error:', error)
    if (executionId) {
      const message = error instanceof Error ? error.message : 'Error desconocido'
      await recordActionComplete(executionId, 'failed', undefined, undefined, message)
    }
