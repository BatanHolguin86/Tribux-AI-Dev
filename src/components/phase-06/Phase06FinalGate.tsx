...
type Phase06FinalGateProps = {
  projectId: string
}

export function Phase06FinalGate({ projectId }: Phase06FinalGateProps) {
  ...
  async function handleApprove() {
    ...
    const res = await fetch(`/api/projects/${projectId}/phases/6/approve`, {
      method: 'POST',
    })
    ...
    toast.success('Phase 06 aprobada — desbloqueando Phase 07...')
    setTimeout(() => {
      router.push(`/projects/${projectId}/phase/07`)
    }, 2000)
  }
  ...
  return (
    <div className="rounded-lg border-2 border-green-200 bg-green-50 p-6 text-center dark:border-green-800 dark:bg-green-900/20">
      ...
      <h2 className="text-lg font-display font-bold text-gray-900 dark:text-gray-100">Phase 06 completada</h2>
      <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
        Has completado las {PHASE06_SECTIONS.length} categorias de Launch & Deployment:
      </p>

      <div className="mx-auto mt-4 max-w-xs space-y-2">
        {PHASE06_SECTIONS.map((section) => (
      ...
      <p className="mt-4 text-xs text-gray-500 dark:text-gray-400">
        Al aprobar, Phase 07 (Iteration & Growth) se desbloqueara automaticamente.
      </p>
