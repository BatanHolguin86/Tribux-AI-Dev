  ...
  return (
    <div
      className={`mx-3 my-2 rounded-xl p-4 text-sm ${
        isCredits
          ? 'border border-brand-amber/30 bg-brand-amber/5'
    ...
    >
      <div className="flex items-start gap-3">
        <div className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-lg ${
          isCredits ? 'bg-brand-amber/10' : 'bg-[#EF4444]/10'
        }`}>
          {isCredits ? '💳' : '⚠️'}
        </div>
        <div className="flex-1 min-w-0">
          <p className={`text-sm font-display font-semibold ${
            isCredits ? 'text-brand-amber' : 'text-[#EF4444]'
          }`}>
            {isCredits ? 'Creditos insuficientes' : 'No se pudo conectar'}
          </p>
          <p className="mt-1 text-xs text-brand-muted dark:text-gray-400">{message}</p>
          {isCredits && (
