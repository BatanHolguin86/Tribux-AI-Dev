    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={handleSync}
            disabled={syncing}
            className="rounded-md bg-brand-primary px-3 py-1.5 text-sm font-medium text-white hover:bg-brand-navy disabled:opacity-50"
          >
      ...
      {loading ? (
        <div className="flex items-center justify-center py-16">
          <div className="h-7 w-7 animate-spin rounded-full border-[3px] border-brand-teal/30 border-t-[#0EA5A3]" />
        </div>
      ) : entries.length === 0 ? (
              ...
              {entries.map((e) => {
                ...
                return (
                  <tr key={e.id} className="bg-white hover:bg-gray-50 dark:bg-gray-900 dark:hover:bg-gray-800">
                    ...
                    <td className="px-4 py-3">
                      <span className="rounded-full bg-brand-surface px-2 py-0.5 text-xs font-medium capitalize text-brand-primary dark:bg-brand-primary/30 dark:text-brand-teal">
                        {e.plan}
                      </span>
                    </td>
