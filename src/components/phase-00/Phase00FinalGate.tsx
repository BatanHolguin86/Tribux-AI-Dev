...
import { SECTION_LABELS, PHASE00_SECTIONS } from '@/lib/ai/prompts/phase-00'

type Phase00FinalGateProps = {
  projectId: string
}

export function Phase00FinalGate({ projectId }: Phase00FinalGateProps) {
  ...
  async function handleApprove() {
    ...
    const res = await fetch(`/api/projects/${projectId}/phases/0/approve`, {
      method: 'POST',
    })
    ...
    setTimeout(() => {
      router.push(`/projects/${projectId}/phase/01`)
    }, 2000)
  }
  ...
  return (
    <div className="rounded-lg border-2 border-green-200 bg-green-50 p-6 text-center dark:border-green-800 dark:bg-green-900/20">
      ...
      <h2 className="text-lg font-display font-bold text-gray-900 dark:text-gray-100">Phase 00 completada</h2>
      <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">
        Has aprobado los 5 documentos del discovery:
      </p>

      <div className="mx-auto mt-4 max-w-xs space-y-2">
        {PHASE00_SECTIONS.map((section) => (
      ...
      <p className="mt-4 text-xs text-gray-500 dark:text-gray-400">
        Al aprobar, Phase 01 (Requirements & Spec) se desbloqueara automaticamente.
      </p>
