...
test.describe('Phase 07 — authenticated', () => {
  ...
  test('Phase 07 loads and shows iteration checklist', async ({ page }) => {
    const projectId = await getProjectId(page)
    await page.goto(`/projects/${projectId}/phase/07`)

    await expect(page.locator('body')).not.toContainText(/error/i, { timeout: 10000 })

    const iterationText = page.getByText(/iteration|growth|checklist|feedback/i)
    const buttons = page.getByRole('button').first()
    const hasContent = await iterationText.isVisible().catch(() => false) ||
