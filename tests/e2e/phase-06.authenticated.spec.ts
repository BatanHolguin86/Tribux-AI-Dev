...
test.describe('Phase 06 — authenticated', () => {
  ...
  test('Phase 06 loads and shows deployment checklist', async ({ page }) => {
    const projectId = await getProjectId(page)
    await page.goto(`/projects/${projectId}/phase/06`)

    await expect(page.locator('body')).not.toContainText(/error/i, { timeout: 10000 })

    const deployText = page.getByText(/deploy|lanzamiento|checklist|launch/i)
    const buttons = page.getByRole('button').first()
    const hasContent = await deployText.isVisible().catch(() => false) ||
