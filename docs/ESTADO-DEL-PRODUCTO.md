...
- **Fases 03–07 (construir + lanzar):** `PhaseProgressHeader`; categorías persistidas en `phase_sections`; **ítems** checklist persistidos en `item_states` (fases 3, 5, 6, 7) además del toggle de categoría. **Phase 04** Kanban (`project_tasks`) como checklist de desarrollo + `PhaseDocsCallout` y cabecera aun sin tasks. **Phase 07** incluye `PhaseDocsCallout` (métricas/brief). Enlaces a `docs/05-qa/` (E2E, go/no-go) y `docs/06-ops/`. **Narrativa IA DLC** en dashboard y layout de proyecto (`DlcClosingNarrative`). **Billing** Stripe opcional según `.env`.
