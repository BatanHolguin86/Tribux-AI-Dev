...
El producto tiene **operativos** los módulos de diseño (Discovery + KIRO + Agentes con adjuntos) y la base técnica (Auth, Dashboard, Supabase, Anthropic). Las fases 03–07 y el Generador de diseños UI/UX tienen especificación y tareas definidas, pero la implementación es parcial o en esqueleto.
...
| Generador UI/UX | ⚠️ Parcial | Vista `/designs` y tabla `design_artifacts`; falta pipeline completo de generación |
...
- **Roadmap KIRO-first** aplicado en tres bloques:
  ...
  - **Bloque 2:** Generador UI/UX con TASK-601–607 (alcance MVP, diseño, migración, API, vista Diseño).
...
- **Vista Diseño:** `/projects/[id]/designs` lee de `design_artifacts` y muestra lista; integrada con `DesignGenerator` (templates + chat UI/UX Designer).
...
- **Generador UI/UX:** Pipeline completo de generación (API `POST /designs/generate`, job/worker o serverless que llame al agente y guarde artefactos en `design_artifacts` y Storage). Hoy la vista lista y el modelo existen; la generación automática desde la UI no está cerrada.
...
2. **Checklist go/no-go en código/docs**  
   Dejar en `docs/05-qa/` o en el PRD una checklist corta de 5–7 ítems (IA configurada, Auth+Onboarding, Phase 00+01, agentes+adjuntos, vista Diseño accesible, E2E críticos pasando) y usarla como criterio de release v1.0.
...
4. **Generador UI/UX mínimo viable**  
   Implementar `POST /api/projects/[id]/designs/generate` (o equivalente) que, con al menos un `design.md` aprobado, invoque al agente UI/UX Designer con contexto KIRO y guarde resultado en `design_artifacts` (y Storage si aplica). Mantener la vista `/designs` como lista + detalle y enlace desde Phase 02/04.
...
- **Avances:** Base metodológica (KIRO + IA DLC), Auth/Dashboard, Phase 00 y 01 completas, Orquestador + 9 agentes con adjuntos y contexto enriquecido, vista Diseño conectada a `design_artifacts`, QA documentada y tests E2E definidos. Correcciones recientes en Phase 00 y chat de agentes para evitar 500 y pantallas mudas.
...
- **Hoja de ruta sugerida:** (A) Estabilidad y mensajes de error unificados + E2E verdes; (B) Generador UI/UX MVP; (C) Checklists 03–07 operativos; (D) Checklist go/no-go, docs de release y lanzamiento. Con esto se puede cerrar una v1.0 alineada al objetivo del producto y estable para pruebas reales.
